...
import pandas as pd
import numpy as np
from utils import explainers, networks, synthetic_data, real_data, check_int_positive, check_float_positive, save_dataframe_csv, load_dataframe_csv
...
def main(args):
    ...
    explainer = explainers[args.explainer](model, args.n_classes, gpu=args.gpu, scale=args.scale, temperature=args.temperature)
    ...
    try:
        df = load_dataframe_csv("tables", "Explainability_temperature_{0}.csv".format(args.data))
    except:
        columns = ['data', 'temperature', 'scale', 'coverage', 'hit_rate', 'execution_time']
    ...
    save_dataframe_csv(df, "tables", "Explainability_temperature_{0}.csv".format(args.data))
